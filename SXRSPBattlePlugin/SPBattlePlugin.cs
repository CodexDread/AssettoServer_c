...
/// </summary>
public class SPBattlePlugin : CriticalBackgroundService, IAssettoServerAutostart
{
    private readonly EntryCarManager _entryCarManager;
    private readonly SPBattleConfiguration _config;
    private readonly Func<EntryCar, EntryCarBattle> _entryCarBattleFactory;
    private readonly LeaderboardService _leaderboard;
    private readonly CSPServerScriptProvider _scriptProvider;
    private readonly Dictionary<int, EntryCarBattle> _instances = new();
    ...
    public SPBattlePlugin(
        EntryCarManager entryCarManager,
        SPBattleConfiguration config,
        Func<EntryCar, EntryCarBattle> entryCarBattleFactory,
        ...
        if (_config.EnableLuaUI)
        {
            try
            {
                string luaPath = Path.Combine(
                    Path.GetDirectoryName(typeof(SPBattlePlugin).Assembly.Location) ?? "",
                    "lua", "spbattle.lua");
                
                if (File.Exists(luaPath))
                {
                    _scriptProvider.AddScript(File.ReadAllText(luaPath), "spbattle.lua");
    ...
    /// </summary>
    internal EntryCarBattle GetBattle(EntryCar entryCar) => _instances[entryCar.SessionId];
    ...
    private void SendBattleStatusEvent(EntryCar car, byte eventType, long eventData)
    {
        car.Client?.SendPacket(new CSPEventPacket
        {
            ...
            Data = new BattleStatusEventData
            {
                EventType = eventType,
                EventData = eventData
            }
    ...
    private void SendBattleUpdateEvent(EntryCar car, Battle battle)
    {
        ...
        car.Client?.SendPacket(new CSPEventPacket
        {
            ...
            Data = new BattleUpdateEventData
            {
