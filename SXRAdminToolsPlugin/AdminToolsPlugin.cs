...
/// </summary>
public class AdminToolsPlugin : CriticalBackgroundService, IAssettoServerAutostart
{
    ...
    private void LoadLuaUI()
    {
        try
        {
            string luaPath = Path.Combine(
                Path.GetDirectoryName(typeof(AdminToolsPlugin).Assembly.Location) ?? "",
                "lua", "admintools.lua");
            
            if (File.Exists(luaPath))
            {
                _scriptProvider.AddScript(File.ReadAllText(luaPath), "admintools.lua");
